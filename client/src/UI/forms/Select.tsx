...
const Select: React.FC<IProp> = ({ data, setValue, label, errors, name, clearErrors, defaultValue, haveBorder = false }) => {
  ...
  return (
    <div className={styles["option-container"]} ref={selectRef}>
      ...
      {isOpen && (
        <div className={styles["option-list"]} style={{ border: haveBorder ? "1px solid var(--black-color)" : "" }}>
          {data.map((el) => (
